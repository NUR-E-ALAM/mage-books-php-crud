...
// $sql = "SELECT b.* FROM books b INNER JOIN writers as w ON (w.id = b.writer_id) WHERE books.id";
$sql = "SELECT * FROM writers INNER JOIN books ON writers.id = books.writer_id";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_array($result)) {?>
        ...
        ?></td>
        <td> <img height='80px' width='100px' src='bookimages/<?php echo $row['images'] ?>'></td>
        <td><?php echo $row["created"] ?></td>
        <td><a href="edit_book.php?id=<?php echo $row['id']; ?>" class="btn btn-primary btn-sm">Edit</a></td>
        <td><a href="delete_book.php?id=<?php echo $row['id']; ?>" onClick="return confirm('Are you absolutely sure you want to delete?')" class="btn btn-danger btn-sm">Remove</a></td>
                        </tr>
                        <?php
    }
    // for writer name start
